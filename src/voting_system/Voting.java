		...
		btnVote.addActionListener(new ActionListener() { 
			public void actionPerformed(ActionEvent e){ 
				...
				try{
					Connection con = DriverManager.getConnection("jdbc:ucanaccess://C:\\Users\\Cedrick Alcantara\\eclipse-workspace\\voting_system\\src\\voting_system\\Database1.accdb");
					Statement stmt = con.createStatement();
					ResultSet rs = stmt.executeQuery("SELECT fname, lname, pos, Tally FROM Candidates");
					...
					while(rs.next()) {
						...
						if(pres.contains(rd)) {
						PreparedStatement upd = con.prepareStatement("UPDATE Candidates "+"SET Tally=? "+ "WHERE lname ='"+rd+"'");
						String prct = rs.getString("Tally");
						...
						if(sec.contains(rd)) {
							PreparedStatement upd = con.prepareStatement("UPDATE Candidates "+"SET Tally=? "+ "WHERE lname ='"+rd+"'");
							String secct = rs.getString("Tally");
						...
						if(vp.contains(rd)) {
							PreparedStatement upd = con.prepareStatement("UPDATE Candidates "+"SET Tally=? "+ "WHERE lname ='"+rd+"'");
							String vpct = rs.getString("Tally");
						...
						if(aud.contains(rd)) {
							PreparedStatement upd = con.prepareStatement("UPDATE Candidates "+"SET Tally=? "+ "WHERE lname ='"+rd+"'");
							String audct = rs.getString("Tally");
						...
						if(tre.contains(rd)) {
							PreparedStatement upd = con.prepareStatement("UPDATE Candidates "+"SET Tally=? "+ "WHERE lname ='"+rd+"'");
							String trect = rs.getString("Tally");
